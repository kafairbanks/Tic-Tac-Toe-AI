...
Board::Board(){
	...
	// this demonstrates a sample game for the splash menu
	moves[0]= 'X';
	DrawBoard();
	moves[5] = 'O';
	napms(1000);
...
// draw board onscreen and print instructions
...
// clear board of moves for new game
...
// returns true if the cell was empty and puts token into place else returns false
bool Board::ValidMove(int row, int col){

	int index = TranslateToIndex(row, col);
	// checks if char is null char to determine if empty
	if (moves[index] == '\0') {
...
//  get absolute row and column coordinates converted from 3-dimensional point relative to game board
vector<int> Board::GetAbsolutePoint(vector<int> point){
	
	// math to turn 3-dimensions into 2-dimensions
	int r, c, z;
	r = BOARD_START_ROW + point[1] * 2;
	c = BOARD_START_COL + point[2] * GRID_WIDTH + point[2] * GRID_SPACE + 4 * point[0]	;
	z = point[2];

	// push row and column coordinates into vector
...
// convert row and column to relative coordinates of the game board
...
// convert index to three-dimensional point
