...
char UI::HandlePlayerInput(){
	...
	{	
		if(board->playerTurn){
			...
			{	
				...
				case 10: // enter
					...
					if(board->ValidMove(board->board_pos.second, board->board_pos.first)){
						...
						// get index  of player move , convert to point and update the AI	
						...
						// if the winning move is populated with four elements, then the game has been
			...
			*/
			...
			// calculate computers move and set board to it
			...
			// traverse moves and check if there are any empty cells, if so then the game can't be a cat's game
			for(int i=0; i<64; i++) {
				if(board->moves[i] != 'X' && board->moves[i] != 'O') {
					break_ch = 1;
					cat = false;
	...
	// check if game over or player cheated
...
void UI::PrintWinningMoves(bool playerWon, bool cheatToWin){
	...
	// checks if cheat code was used to toggle boolean
	if(cheatToWin)
		playerWon = true;
		
	vector< vector <int> > absolutePoints;
	
	// if cheat code wasn't used then add the winning moves to a vector that'll later blink
	if(!cheatToWin){
		for(int i = 0; i < 4; ++i){
	...
	if(playerWon){
		color = 2;
		msg = "YOU WIN";
		// checks if cheat code was used to win
		if(cheatToWin)
			msg += " BUT CHEATED";
...
// A constraint is used in order to maintain the cursor in the grids
bool UI::MoveCursor(int move_y, int move_x){
	int cur_x, cur_y;
	getyx(win, cur_y, cur_x);
	
	// the modulo operator is used to determine if the move needs to be increased or decreased to cover
	// gap between each grid
...
// called when program is initialized
void UI::StartUI(){
	Refresh();
	SplashScreen();
	PrintInstructions();	
...
// calls ncurses window refresh function and redraws box around the game
void UI::Refresh(){		
		box(win, '|', '*');
		wrefresh(win);
}
